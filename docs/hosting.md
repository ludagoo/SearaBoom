...
USB factory for testers is the curl one-liner on the public flash page (TUI binary from `/api/factory/flasher/`; firmware fetched live from Flask). Chrome WebSerial on that page is a fallback. Mini-local admin flash is `POST /api/factory/flash` and needs a box on `SEARABOOM_SERIAL_PORT`.

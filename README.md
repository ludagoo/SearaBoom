...
|-----------|------------|-----------------|
| New or bricked unit | Desktop flasher: one binary from https://searaboom.goossen.dev/. Linux: `chmod +x` the downloaded file (browsers drop `+x`), then run it. ARM, plug USB. Fetches the live USB image. DTR/RTS, no BOOT button. Chrome WebSerial is a fallback. | Manual flash (new/bricked units) |
...
Factory flash (new boxes): download one factory-flasher binary from https://searaboom.goossen.dev/. Linux browsers drop `+x` — `chmod +x` the file, then run it, or use the homepage curl one-liner. Chrome WebSerial remains a fallback.  

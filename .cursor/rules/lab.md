...
- Never create `~/.config/searaboom/signing_key_backed_up`. Only Lucas creates that file after the signing key is stored offline. Until it exists, refuse signed USB flash and OTA/factory publish (`docs/SIGNED_FIRMWARE.md`). No `--force`; policy forbids env tricks.

#!/usr/bin/env bash
# Bump firmware patch version (X.Y.Z), rebuild, publish OTA and USB factory
# as the same version.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  Bumps patch in firmware/VERSION, builds, publishes OTA and USB factory."
      exit 0
      ;;
...
"$ROOT/scripts/publish_firmware.sh" "$NEW"

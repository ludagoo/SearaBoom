...
def test_fill_policy_unchanged() -> None:
    assert VER == "0.5.29"
    assert 'CONFIG_SEARABOOM_FW_VERSION="0.5.29"' in SDK
    ...
    assert "CONFIG_LOG_MAXIMUM_EQUALS_DEFAULT=y" in SDK
    assert "CONFIG_LOG_MAXIMUM_LEVEL_INFO=y" not in SDK
    ...
    assert "CONFIG_LOG_MAXIMUM_LEVEL_(DEBUG|VERBOSE)=y" in cmake
    assert "CONFIG_LOG_MAXIMUM_LEVEL_INFO=y" not in cmake

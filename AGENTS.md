...
- USB factory is **0.0.9** (full semver on boot). OTA latest is **0.1.0**. Firmware after `0.1.0` applies any newer `X.Y.Z` on boot. Do not bump only the patch vs a `0.0.63` factory image — that build ignored patch.

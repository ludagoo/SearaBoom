...
Do **not** point the public flash page at `firmware/build/`. Factory files live in `server/firmware/factory/`. USB install for testers is the **desktop flasher** from https://searaboom.goossen.dev/ (`factory_flasher/`, esptool DTR/RTS). Chrome WebSerial is a fallback. `idf.py flash` is local-dev only, never on QA nodes.

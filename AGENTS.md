...
- USB factory is **0.1.0**. OTA latest is **0.2.0**. Firmware after `0.1.0` applies any newer `X.Y.Z` on boot. Do not bump only the patch vs a `0.0.63` factory image — that build ignored patch.
